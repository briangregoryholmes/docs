...
Finally, Edge connetions can be specified at runtime by passing an array of tuples via the connections prop. Each tuple represents a corresponding user-defined Node and Anchor ID.
...
>
  When not passing a custom Edge component, this prop can be used to set the
  color for all Edges initiated from the Anchor
</ResponseField>
